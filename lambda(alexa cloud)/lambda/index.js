...
const WeatherForecastIntentHandler = {
    ...
    try {
      ...
      speakOutput = d.toString().replace(/\s*\[†\d+\]/g, '').replace(/\[†[0-9]+(, ?†[0-9]+)*\]/g, '').replace(/■DONT_KNOW/g, '');
    } catch (error) {
      speakOutput = "Internal server error";
...
const WeatherIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    try {
      ...
      const d = response.data;
      if(typeof d === "string"){
          speakOutput = d.replace(/\s*\[†\d+\]/g, '').replace(/\[†[0-9]+(, ?†[0-9]+)*\]/g, '').replace(/■DONT_KNOW/g, '').replace(/: Source: Weather API/g, '');
      } else {
         speakOutput = d[0].replace(/\s*\[†\d+\]/g, '').replace(/\[†[0-9]+(, ?†[0-9]+)*\]/g, '').replace(/■DONT_KNOW/g, '');        
      }

    } catch (error) {
      speakOutput = "error "+JSON.stringify(error);
      if (error.message === "Request failed with status code 400") {
        speakOutput = "Location not found, please try another.";
      }
    }
